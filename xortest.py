
    
def xorb(a,b):
    return bytes(x^y for x,y in zip(a,b))

def getCipherList():
    ct1 = ""
    ct2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
    ct3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
    ct4 = ""
    ct5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
    ct6 = ""
    ct7 = ""
    ct8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
    ct9 = ""
    ct10 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
    target = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"
    ciphers = [ct1,ct2,ct3,ct4,ct5,ct6,ct7,ct8,ct9,ct10,target]
    return ciphers


def isOneSpace(a):
    # check if a is in a-zA-Z range
    return ((a >= 65 and a <= 90) or (a >= 97 and a <= 122))

def calculateScore(list):
    list = getCipherList()
    rows, cols = 11,83
    score = [[0 for x in range(cols)] for y in range(rows)] 
    for i in range(rows):
       for j in range(i+1, rows):
          msg1 = bytes.fromhex(list[i])
          msg2 = bytes.fromhex(list[j])
          outmsg = xorb(msg1,msg2)
          # iterate each code in the xored output
          for k in range(cols):
            if (isOneSpace(outmsg[k])):
                score[i][k] += 1
                score[j][k] += 1
    return score
            
def getCipherWithSpaceAtIndex(score):
    rows, cols = 11,83
    pos=[0 for x in range(cols)] #contains index of ciphertext in the list which have the space at the index 
    for i in range(cols):
        maxScore = 0
        listIndex = 0
        for j in range(rows):
            if (score[j][i] > maxScore):
               listIndex  = j
               maxScore = score[j][i]
        pos[i]=listIndex
    return pos

# get the cipher keys as byte array
def getKeys(pos, list):
    keys=[0 for x in range(83)]
    for i in range(len(pos)):
        cipher = bytes.fromhex(list[pos[i]])
        key = xorb(bytes.fromhex('20'), bytes([cipher[i]]))
        keys[i]=key
    return keys

def decrypt(target,keys):
    targetBytes = bytes.fromhex(target)
    result = [0 for x in range(83)]
    for i in range(len(targetBytes)):
        code = bytes([targetBytes[i]])
        key = keys[i]
        decrypted = xorb(code,key)
        charVal = decrypted.decode()
        result[i]=charVal
    listToStr = ''.join([str(elem) for elem in result])
    return listToStr

    
def main():
    list = getCipherList()
    score = calculateScore(list)
    pos = getCipherWithSpaceAtIndex(score)
    keys =getKeys(pos,list)
    encryptedTarget = list[10]
    target = decrypt(encryptedTarget,keys)
    print(target)

main()